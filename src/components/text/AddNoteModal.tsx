...
export function AddNoteModal({ open, onClose, discriminator }: AddNoteModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      await createNote({
        discriminatorId: discriminator._id,
        differentialIndex,
        field,
              ...
              <DialogPanel className="w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden">
                <div className="bg-slate-900 px-6 py-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-slate-400 text-xs mt-0.5 font-medium">{discriminator.pattern}</p>
                  </div>
