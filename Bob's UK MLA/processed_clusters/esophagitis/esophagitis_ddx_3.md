# Finding: Esophagitis (DDx)

**Source section:** Esophagitis
**Quality score:** 0.5
**Differential count:** 2

## Differentials

- herpes esophagitis (immunosuppressed patient
- less localized); reflux esophagitis

## Context

EN FACE: central punctate collections of barium surrounded by a radiolucent halo (= | IN PROFILE: elongated flat plaquelike filling defects | superficial solitary / several discrete ulcers with sharply delineated margins with | ulcers may be invisible

---
<!--
  LLM EXTRACTION INSTRUCTIONS (Stage 2):
  1. Convert ## Differentials into a comparison table.
  2. Columns = each differential diagnosis.
  3. Rows = Dominant finding | Demographics | Distribution |
            Key discriminator | Other findings | Relative frequency.
  4. Preserve any percentages or 'most common/rare' qualifiers.
  5. Use ## Context paragraph for additional demographic / distribution clues.
-->
