# Finding: Pituitary Apoplexy (DDx)

**Source section:** Pituitary Apoplexy
**Quality score:** 0.5
**Differential count:** 2

## Differentials

- Rathke cleft cyst (NO hemorrhage
- NO fluid-fluid level)

## Context

high signal on DWI (in acute stage) ← restricted diffusion | predominant T1 hyperintensity + predominant T2 hypointensity ← hemoglobin | intermediate SI on T1WI + T2WI ← deoxyhemoglobin | intratumoral fluid-debris level ← sedimentation of blood products (at later stage)

---
<!--
  LLM EXTRACTION INSTRUCTIONS (Stage 2):
  1. Convert ## Differentials into a comparison table.
  2. Columns = each differential diagnosis.
  3. Rows = Dominant finding | Demographics | Distribution |
            Key discriminator | Other findings | Relative frequency.
  4. Preserve any percentages or 'most common/rare' qualifiers.
  5. Use ## Context paragraph for additional demographic / distribution clues.
-->
